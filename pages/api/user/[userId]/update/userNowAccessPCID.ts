// File: pages/api/user/[userId]/update/userNowAccessPCID.ts
...
export default async function handler(
  ...
  const { userNowAccessPCID } = req.body;
  ...
  try {
    ...
    const result = await usersCollection.updateOne(
      ...
      { $set: { userNowAccessPCID } },
    );

    if (!result) {
      return res
        .status(400)
        .json(respond('Failed to update userNowAccessPCID'));
    } else {
